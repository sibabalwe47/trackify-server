...
exports.addHabit = asynHandler(async (req, res) => {
    ...
    if(habits.length > 0) return res.status(401).json({
        success: false,
...
exports.getAll = asynHandler(async (req, res) => {
    ...
    if(results.length == 0) return res.status(401).send({ success: false, message: 'User has no recorded habits yet.' })
    res.json({
        success: true,
        data: results
    })
...
exports.getSingle = asynHandler(async (req, res) => {
    ...
    if(results.length == 0) return res.status(401).send({ success: false, message: 'Habit does not exist.' })
    res.json({
        success: true,
        data: {
            id: results[0].id,
