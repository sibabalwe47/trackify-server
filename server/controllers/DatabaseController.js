...
class Database {
  ...
  async findAll(columns) {
    ...
    const [rows, fields] = await conn.query(
      `SELECT * FROM ${this.table} WHERE ${query.trimEnd().slice(0, -3)}`
    );
    return rows;
  }
  ...
  async findOne(columns) {
    ...
    const conn = await this.connection;
    const [rows, fields] = await conn.query(
      `SELECT * from ${this.table} WHERE ${optionsString
        .trimEnd()
        .slice(0, -3)}`
    );
    return rows;
  ...
  async findOneById(id, field, join) {
    const conn = await this.connection;
    const [rows, fields] = await conn.query(
      `SELECT * FROM ${this.table} WHERE ${field.name} = '${field.id}' AND ${join} = ${id}`
    );
    return rows;
  }
  ...
  async findByRange(columns) {
    ...
    const [rows, fields] = await conn.query(
      `SELECT * FROM ${this.table} WHERE ${
        columns.field
      } BETWEEN ${optionsString.trimEnd().slice(0, -3)}`
    );
    return rows;
  }
  ...
  async update(columns) {
    ...
    const conn = await this.connection;
    const [rows, fields] = await conn.query(
      `UPDATE ${this.table} SET ${setString
        .trimEnd()
        .slice(0, -1)} WHERE ${optionsString.trimEnd().slice(0, -3)}`
    );
    return rows;
  }
  ...
  async delete(columns) {
    ...
    const conn = await this.connection;
    const [rows, fields] = await conn.query(
      `DELETE FROM ${this.table} WHERE ${query.trimEnd().slice(0, -3)}`
    );
    return rows;
  }
}
